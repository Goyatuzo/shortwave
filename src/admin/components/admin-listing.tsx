...
export class AdminListingComponent extends React.Component<AdminListingProps, AdminListingState> {
    ...
    componentWillMount() {
        ...
        itemsRef.on('child_added', snapshot => {
            ...
            this.setState({
                items: [...this.state.items, { key: snapshot.key, name: value.name, video: value.video }]
            });
        });
    }
    ...
    render() {
        return (
            <div>
                {
                    this.state.items.map(item => <GalleryItemView key={item.key} caption={item.name} youtubeId={item.video} />)
                }
            </div>
        )
    }
